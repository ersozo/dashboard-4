...
function calculateUnitMetrics(unitName) {
    ...
    const quality = totalQty > 0 ? weightedQualitySum / totalQty : 0;
...
function createCharts() {
    ...
    productionChart = new Chart(productionCtx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Toplam Üretim',
                data: unitMetrics.map(m => m.production),
...
function updateCharts() {
    ...
    // Update chart data
    productionChart.data.datasets[0].data = unitMetrics.map(m => m.production);
    ...
    oeeChart.data.datasets[0].data = unitMetrics.map(m => m.oee);
...
function updateSummaryStatistics(unitMetrics) {
    const totalProduction = unitMetrics.reduce((sum, m) => sum + m.production, 0);
    ...
    const avgOEE = unitMetrics.length > 0 ? 
        unitMetrics.reduce((sum, m) => sum + m.oee, 0) / unitMetrics.length : 0;
    ...
    const oldTotalProduction = totalProductionElement.textContent;
    const newTotalProduction = totalProduction.toLocaleString();
    if (oldTotalProduction !== newTotalProduction) {
        totalProductionElement.textContent = newTotalProduction;
        elementsToFlashOnUpdate.push(totalProductionElement);
    }
    ...
    const avgOEEElement = document.getElementById('avg-oee');
    const oldAvgOEE = avgOEEElement.textContent;
    const newAvgOEE = avgOEE.toFixed(1) + '%';
    if (oldAvgOEE !== newAvgOEE) {
        avgOEEElement.textContent = newAvgOEE;
        elementsToFlashOnUpdate.push(avgOEEElement);
    }
} 