...
from database import get_production_units, get_production_data, get_db_connection, TIMEZONE
import pytz
    ...
    try:
        while True:
            try:
                ...
                production_data = get_production_data(unit_name, start_time, end_time, current_time)
        ...
        while True:
            try:
                ...
                if models_with_target:
                    operation_time = (current_time - start_time).total_seconds() if current_time > start_time else 0
                    ...
                    # print(f"Total operation time: {operation_time:.2f} seconds")
                    # print(f"Total performance: {total_performance:.4f}" if total_performance is not None else "Total performance: None")
                    # print(f"Total OEE: {total_oee:.4f}" if total_oee is not None else "Total OEE: None")
                else:
                ...
                for row in all_records:
                    ...
                    hourly_models[hour_key][model_name]['total_qty'] += 1
                    ...
                    hour_containers[hour_key]['total_qty'] += 1
                ...
                for hour_key, container in hour_containers.items():
                    ...
                    if hour_model_list:
                        ...
                        if models_with_target:
                            ...
                            if is_current:
                                operation_time = (current_time - hour_start).total_seconds()
                            else:
                                operation_time = (hour_end - hour_start).total_seconds()
